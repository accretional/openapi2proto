...
func NewAnysOrExpressions(in *yaml.Node, context *compiler.Context) (*AnysOrExpressions, error) {
	errors := make([]error, 0)
	x := &AnysOrExpressions{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
		...
		for i := 0; i < len(m.Content); i += 2 {
			...
			if ok {
				...
				pair.Name = k
				var err error
				pair.Value, err = NewAnyOrExpression(v, compiler.NewContext(k, v, context))
				if err != nil {
...
func NewComponents(in *yaml.Node, context *compiler.Context) (*Components, error) {
	...
	if !ok {
		...
		allowedKeys := []string{"callbacks", "examples", "headers", "links", "parameters", "pathItems", "requestBodies", "responses", "schemas", "securitySchemes"}
		...
		// PathItems path_items = 10;
		v10 := compiler.MapValueForKey(m, "pathItems")
		if v10 != nil {
			var err error
			x.PathItems, err = NewPathItems(v10, compiler.NewContext("pathItems", v10, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// repeated NamedAny specification_extension = 11;
		// MAP: Any ^x-
		x.SpecificationExtension = make([]*NamedAny, 0)
		for i := 0; i < len(m.Content); i += 2 {
			k, ok := compiler.StringForScalarNode(m.Content[i])
...
// NewDependentRequired creates an object of type DependentRequired if possible, returning an error if not.
func NewDependentRequired(in *yaml.Node, context *compiler.Context) (*DependentRequired, error) {
	errors := make([]error, 0)
	x := &DependentRequired{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
...
// NewDependentSchemas creates an object of type DependentSchemas if possible, returning an error if not.
func NewDependentSchemas(in *yaml.Node, context *compiler.Context) (*DependentSchemas, error) {
	errors := make([]error, 0)
	x := &DependentSchemas{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
...
func NewDocument(in *yaml.Node, context *compiler.Context) (*Document, error) {
	...
	if !ok {
		...
		requiredKeys := []string{"info", "openapi"}
		missingKeys := compiler.MissingKeysInMap(m, requiredKeys)
		if len(missingKeys) > 0 {
			message := fmt.Sprintf("is missing required %s: %+v", compiler.PluralProperties(len(missingKeys)), strings.Join(missingKeys, ", "))
			errors = append(errors, compiler.NewError(context, message))
		}
		allowedKeys := []string{"components", "externalDocs", "info", "jsonSchemaDialect", "openapi", "paths", "security", "servers", "tags", "webhooks"}
		allowedPatterns := []*regexp.Regexp{pattern1}
		invalidKeys := compiler.InvalidKeysInMap(m, allowedKeys, allowedPatterns)
		if len(invalidKeys) > 0 {
			message := fmt.Sprintf("has invalid %s: %+v", compiler.PluralProperties(len(invalidKeys)), strings.Join(invalidKeys, ", "))
		...
		v3 := compiler.MapValueForKey(m, "jsonSchemaDialect")
		if v3 != nil {
			x.JsonSchemaDialect, ok = compiler.StringForScalarNode(v3)
			if !ok {
				message := fmt.Sprintf("has unexpected value for jsonSchemaDialect: %s", compiler.Display(v3))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// repeated Server servers = 4;
		v4 := compiler.MapValueForKey(m, "servers")
		if v4 != nil {
			// repeated Server
			x.Servers = make([]*Server, 0)
			a, ok := compiler.SequenceNodeForNode(v4)
			if ok {
		...
		// Paths paths = 5;
		v5 := compiler.MapValueForKey(m, "paths")
		if v5 != nil {
			var err error
			x.Paths, err = NewPaths(v5, compiler.NewContext("paths", v5, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// PathItems webhooks = 6;
		v6 := compiler.MapValueForKey(m, "webhooks")
		if v6 != nil {
			var err error
			x.Webhooks, err = NewPathItems(v6, compiler.NewContext("webhooks", v6, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// Components components = 7;
		v7 := compiler.MapValueForKey(m, "components")
		if v7 != nil {
			var err error
			x.Components, err = NewComponents(v7, compiler.NewContext("components", v7, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// repeated SecurityRequirement security = 8;
		v8 := compiler.MapValueForKey(m, "security")
		if v8 != nil {
		...
		// repeated Tag tags = 9;
		v9 := compiler.MapValueForKey(m, "tags")
		if v9 != nil {
			// repeated Tag
			x.Tags = make([]*Tag, 0)
		...
		v10 := compiler.MapValueForKey(m, "externalDocs")
		if v10 != nil {
			var err error
			x.ExternalDocs, err = NewExternalDocs(v10, compiler.NewContext("externalDocs", v10, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
...
func NewHeader(in *yaml.Node, context *compiler.Context) (*Header, error) {
	...
	if !ok {
		...
		allowedKeys := []string{"content", "deprecated", "description", "example", "examples", "explode", "required", "schema", "style"}
		allowedPatterns := []*regexp.Regexp{pattern1}
		invalidKeys := compiler.InvalidKeysInMap(m, allowedKeys, allowedPatterns)
		if len(invalidKeys) > 0 {
			message := fmt.Sprintf("has invalid %s: %+v", compiler.PluralProperties(len(invalidKeys)), strings.Join(invalidKeys, ", "))
		...
		// string style = 4;
		v4 := compiler.MapValueForKey(m, "style")
		if v4 != nil {
			x.Style, ok = compiler.StringForScalarNode(v4)
			if !ok {
				message := fmt.Sprintf("has unexpected value for style: %s", compiler.Display(v4))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// bool explode = 5;
		v5 := compiler.MapValueForKey(m, "explode")
		if v5 != nil {
			x.Explode, ok = compiler.BoolForScalarNode(v5)
			if !ok {
				message := fmt.Sprintf("has unexpected value for explode: %s", compiler.Display(v5))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// SchemaOrReference schema = 6;
		v6 := compiler.MapValueForKey(m, "schema")
		if v6 != nil {
			var err error
			x.Schema, err = NewSchemaOrReference(v6, compiler.NewContext("schema", v6, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		...
		// ExamplesOrReferences examples = 8;
		v8 := compiler.MapValueForKey(m, "examples")
		if v8 != nil {
			var err error
			x.Examples, err = NewExamplesOrReferences(v8, compiler.NewContext("examples", v8, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// MediaTypes content = 9;
		v9 := compiler.MapValueForKey(m, "content")
		if v9 != nil {
			var err error
			x.Content, err = NewMediaTypes(v9, compiler.NewContext("content", v9, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
...
func NewInfo(in *yaml.Node, context *compiler.Context) (*Info, error) {
	...
	if !ok {
		...
		// string version = 7;
		v7 := compiler.MapValueForKey(m, "version")
		if v7 != nil {
			x.Version, ok = compiler.StringForScalarNode(v7)
			if !ok {
				message := fmt.Sprintf("has unexpected value for version: %s", compiler.Display(v7))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
...
func NewLicense(in *yaml.Node, context *compiler.Context) (*License, error) {
	...
	if !ok {
		...
		allowedKeys := []string{"identifier", "name", "url"}
		allowedPatterns := []*regexp.Regexp{pattern1}
		invalidKeys := compiler.InvalidKeysInMap(m, allowedKeys, allowedPatterns)
		if len(invalidKeys) > 0 {
			message := fmt.Sprintf("has invalid %s: %+v", compiler.PluralProperties(len(invalidKeys)), strings.Join(invalidKeys, ", "))
		...
		// string identifier = 2;
		v2 := compiler.MapValueForKey(m, "identifier")
		if v2 != nil {
			x.Identifier, ok = compiler.StringForScalarNode(v2)
			if !ok {
				message := fmt.Sprintf("has unexpected value for identifier: %s", compiler.Display(v2))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string url = 3;
		...
		if v3 != nil {
			x.Url, ok = compiler.StringForScalarNode(v3)
			if !ok {
				message := fmt.Sprintf("has unexpected value for url: %s", compiler.Display(v3))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// repeated NamedAny specification_extension = 4;
...
func NewLink(in *yaml.Node, context *compiler.Context) (*Link, error) {
	...
	if !ok {
		...
		if v3 != nil {
			var err error
			x.Parameters, err = NewAnysOrExpressions(v3, compiler.NewContext("parameters", v3, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
...
func NewNamedAnyOrExpression(in *yaml.Node, context *compiler.Context) (*NamedAnyOrExpression, error) {
	errors := make([]error, 0)
	...
	if !ok {
		...
		if v2 != nil {
			var err error
			x.Value, err = NewAnyOrExpression(v2, compiler.NewContext("value", v2, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
...
// NewPathItems creates an object of type PathItems if possible, returning an error if not.
func NewPathItems(in *yaml.Node, context *compiler.Context) (*PathItems, error) {
	errors := make([]error, 0)
	x := &PathItems{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
		...
		// repeated NamedPathItem additional_properties = 1;
		// MAP: PathItem
		x.AdditionalProperties = make([]*NamedPathItem, 0)
		for i := 0; i < len(m.Content); i += 2 {
			k, ok := compiler.StringForScalarNode(m.Content[i])
			if ok {
				v := m.Content[i+1]
...
func NewSchema(in *yaml.Node, context *compiler.Context) (*Schema, error) {
	...
	if !ok {
		...
		allowedKeys := []string{"$anchor", "$comment", "$defs", "$dynamicAnchor", "$dynamicRef", "$id", "$recursiveAnchor", "$recursiveRef", "$schema", "$vocabulary", "additionalProperties", "allOf", "anyOf", "const", "contains", "contentEncoding", "contentMediaType", "default", "dependentRequired", "dependentSchemas", "deprecated", "description", "discriminator", "else", "enum", "example", "examples", "exclusiveMaximum", "exclusiveMinimum", "externalDocs", "format", "if", "items", "maxItems", "maxLength", "maxProperties", "maximum", "minItems", "minLength", "minProperties", "minimum", "multipleOf", "not", "nullable", "oneOf", "pattern", "prefixItems", "properties", "propertyNames", "readOnly", "required", "then", "title", "type", "unevaluatedItems", "unevaluatedProperties", "uniqueItems", "writeOnly", "xml"}
		allowedPatterns := []*regexp.Regexp{pattern1}
		invalidKeys := compiler.InvalidKeysInMap(m, allowedKeys, allowedPatterns)
		if len(invalidKeys) > 0 {
			message := fmt.Sprintf("has invalid %s: %+v", compiler.PluralProperties(len(invalidKeys)), strings.Join(invalidKeys, ", "))
		...
		v1 := compiler.MapValueForKey(m, "discriminator")
		if v1 != nil {
			var err error
			x.Discriminator, err = NewDiscriminator(v1, compiler.NewContext("discriminator", v1, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// Xml xml = 2;
		v2 := compiler.MapValueForKey(m, "xml")
		if v2 != nil {
			var err error
			x.Xml, err = NewXml(v2, compiler.NewContext("xml", v2, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		...
		// Any example = 4;
		v4 := compiler.MapValueForKey(m, "example")
		if v4 != nil {
			var err error
			x.Example, err = NewAny(v4, compiler.NewContext("example", v4, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// string title = 5;
		v5 := compiler.MapValueForKey(m, "title")
		if v5 != nil {
			x.Title, ok = compiler.StringForScalarNode(v5)
			if !ok {
				message := fmt.Sprintf("has unexpected value for title: %s", compiler.Display(v5))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// float multiple_of = 6;
		...
		// string type = 21;
		v21 := compiler.MapValueForKey(m, "type")
		if v21 != nil {
			x.Type, ok = compiler.StringForScalarNode(v21)
			if !ok {
				message := fmt.Sprintf("has unexpected value for type: %s", compiler.Display(v21))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// repeated SchemaOrReference all_of = 22;
		v22 := compiler.MapValueForKey(m, "allOf")
		if v22 != nil {
			// repeated SchemaOrReference
			x.AllOf = make([]*SchemaOrReference, 0)
			a, ok := compiler.SequenceNodeForNode(v22)
			if ok {
				for _, item := range a.Content {
		...
		// repeated SchemaOrReference one_of = 23;
		v23 := compiler.MapValueForKey(m, "oneOf")
		if v23 != nil {
			// repeated SchemaOrReference
			x.OneOf = make([]*SchemaOrReference, 0)
			a, ok := compiler.SequenceNodeForNode(v23)
			if ok {
				for _, item := range a.Content {
		...
		// repeated SchemaOrReference any_of = 24;
		v24 := compiler.MapValueForKey(m, "anyOf")
		if v24 != nil {
			// repeated SchemaOrReference
		...
		// Schema not = 25;
		v25 := compiler.MapValueForKey(m, "not")
		if v25 != nil {
			var err error
			x.Not, err = NewSchema(v25, compiler.NewContext("not", v25, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// ItemsItem items = 26;
		v26 := compiler.MapValueForKey(m, "items")
		if v26 != nil {
			var err error
			x.Items, err = NewItemsItem(v26, compiler.NewContext("items", v26, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// Properties properties = 27;
		v27 := compiler.MapValueForKey(m, "properties")
		if v27 != nil {
			var err error
			x.Properties, err = NewProperties(v27, compiler.NewContext("properties", v27, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// AdditionalPropertiesItem additional_properties = 28;
		v28 := compiler.MapValueForKey(m, "additionalProperties")
		if v28 != nil {
			var err error
			x.AdditionalProperties, err = NewAdditionalPropertiesItem(v28, compiler.NewContext("additionalProperties", v28, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// DefaultType default = 29;
		v29 := compiler.MapValueForKey(m, "default")
		if v29 != nil {
			var err error
			x.Default, err = NewDefaultType(v29, compiler.NewContext("default", v29, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// string description = 30;
		v30 := compiler.MapValueForKey(m, "description")
		if v30 != nil {
			x.Description, ok = compiler.StringForScalarNode(v30)
			if !ok {
				message := fmt.Sprintf("has unexpected value for description: %s", compiler.Display(v30))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string format = 31;
		v31 := compiler.MapValueForKey(m, "format")
		if v31 != nil {
			x.Format, ok = compiler.StringForScalarNode(v31)
			if !ok {
				message := fmt.Sprintf("has unexpected value for format: %s", compiler.Display(v31))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// DefaultType const = 32;
		v32 := compiler.MapValueForKey(m, "const")
		if v32 != nil {
			var err error
			x.Const, err = NewDefaultType(v32, compiler.NewContext("const", v32, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// string _id = 33;
		v33 := compiler.MapValueForKey(m, "$id")
		if v33 != nil {
			x.XId, ok = compiler.StringForScalarNode(v33)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $id: %s", compiler.Display(v33))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _anchor = 34;
		v34 := compiler.MapValueForKey(m, "$anchor")
		if v34 != nil {
			x.XAnchor, ok = compiler.StringForScalarNode(v34)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $anchor: %s", compiler.Display(v34))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _comment = 35;
		v35 := compiler.MapValueForKey(m, "$comment")
		if v35 != nil {
			x.XComment, ok = compiler.StringForScalarNode(v35)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $comment: %s", compiler.Display(v35))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _schema = 36;
		v36 := compiler.MapValueForKey(m, "$schema")
		if v36 != nil {
			x.XSchema, ok = compiler.StringForScalarNode(v36)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $schema: %s", compiler.Display(v36))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _dynamic_ref = 37;
		v37 := compiler.MapValueForKey(m, "$dynamicRef")
		if v37 != nil {
			x.XDynamicRef, ok = compiler.StringForScalarNode(v37)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $dynamicRef: %s", compiler.Display(v37))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _dynamic_anchor = 38;
		v38 := compiler.MapValueForKey(m, "$dynamicAnchor")
		if v38 != nil {
			x.XDynamicAnchor, ok = compiler.StringForScalarNode(v38)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $dynamicAnchor: %s", compiler.Display(v38))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string _recursive_ref = 39;
		v39 := compiler.MapValueForKey(m, "$recursiveRef")
		if v39 != nil {
			x.XRecursiveRef, ok = compiler.StringForScalarNode(v39)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $recursiveRef: %s", compiler.Display(v39))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// bool _recursive_anchor = 40;
		v40 := compiler.MapValueForKey(m, "$recursiveAnchor")
		if v40 != nil {
			x.XRecursiveAnchor, ok = compiler.BoolForScalarNode(v40)
			if !ok {
				message := fmt.Sprintf("has unexpected value for $recursiveAnchor: %s", compiler.Display(v40))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// XVocabulary _vocabulary = 41;
		v41 := compiler.MapValueForKey(m, "$vocabulary")
		if v41 != nil {
			var err error
			x.XVocabulary, err = NewXVocabulary(v41, compiler.NewContext("$vocabulary", v41, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// XDefs _defs = 42;
		v42 := compiler.MapValueForKey(m, "$defs")
		if v42 != nil {
			var err error
			x.XDefs, err = NewXDefs(v42, compiler.NewContext("$defs", v42, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// SchemaOrReference property_names = 43;
		v43 := compiler.MapValueForKey(m, "propertyNames")
		if v43 != nil {
			var err error
			x.PropertyNames, err = NewSchemaOrReference(v43, compiler.NewContext("propertyNames", v43, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// repeated SchemaOrReference prefix_items = 44;
		v44 := compiler.MapValueForKey(m, "prefixItems")
		if v44 != nil {
			// repeated SchemaOrReference
			x.PrefixItems = make([]*SchemaOrReference, 0)
			a, ok := compiler.SequenceNodeForNode(v44)
			if ok {
				for _, item := range a.Content {
					y, err := NewSchemaOrReference(item, compiler.NewContext("prefixItems", item, context))
					if err != nil {
						errors = append(errors, err)
					}
					x.PrefixItems = append(x.PrefixItems, y)
				}
			}
		}
		// SchemaOrReference contains = 45;
		v45 := compiler.MapValueForKey(m, "contains")
		if v45 != nil {
			var err error
			x.Contains, err = NewSchemaOrReference(v45, compiler.NewContext("contains", v45, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// SchemaOrReference if = 46;
		v46 := compiler.MapValueForKey(m, "if")
		if v46 != nil {
			var err error
			x.If, err = NewSchemaOrReference(v46, compiler.NewContext("if", v46, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// SchemaOrReference then = 47;
		v47 := compiler.MapValueForKey(m, "then")
		if v47 != nil {
			var err error
			x.Then, err = NewSchemaOrReference(v47, compiler.NewContext("then", v47, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// SchemaOrReference else = 48;
		v48 := compiler.MapValueForKey(m, "else")
		if v48 != nil {
			var err error
			x.Else, err = NewSchemaOrReference(v48, compiler.NewContext("else", v48, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// DependentSchemas dependent_schemas = 49;
		v49 := compiler.MapValueForKey(m, "dependentSchemas")
		if v49 != nil {
			var err error
			x.DependentSchemas, err = NewDependentSchemas(v49, compiler.NewContext("dependentSchemas", v49, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// DependentRequired dependent_required = 50;
		v50 := compiler.MapValueForKey(m, "dependentRequired")
		if v50 != nil {
			var err error
			x.DependentRequired, err = NewDependentRequired(v50, compiler.NewContext("dependentRequired", v50, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// string content_encoding = 51;
		v51 := compiler.MapValueForKey(m, "contentEncoding")
		if v51 != nil {
			x.ContentEncoding, ok = compiler.StringForScalarNode(v51)
			if !ok {
				message := fmt.Sprintf("has unexpected value for contentEncoding: %s", compiler.Display(v51))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// string content_media_type = 52;
		v52 := compiler.MapValueForKey(m, "contentMediaType")
		if v52 != nil {
			x.ContentMediaType, ok = compiler.StringForScalarNode(v52)
			if !ok {
				message := fmt.Sprintf("has unexpected value for contentMediaType: %s", compiler.Display(v52))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// UnevaluatedItemsItem unevaluated_items = 53;
		v53 := compiler.MapValueForKey(m, "unevaluatedItems")
		if v53 != nil {
			var err error
			x.UnevaluatedItems, err = NewUnevaluatedItemsItem(v53, compiler.NewContext("unevaluatedItems", v53, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// UnevaluatedPropertiesItem unevaluated_properties = 54;
		v54 := compiler.MapValueForKey(m, "unevaluatedProperties")
		if v54 != nil {
			var err error
			x.UnevaluatedProperties, err = NewUnevaluatedPropertiesItem(v54, compiler.NewContext("unevaluatedProperties", v54, context))
			if err != nil {
				errors = append(errors, err)
			}
		}
		// repeated DefaultType examples = 55;
		v55 := compiler.MapValueForKey(m, "examples")
		if v55 != nil {
			// repeated DefaultType
			x.Examples = make([]*DefaultType, 0)
			a, ok := compiler.SequenceNodeForNode(v55)
			if ok {
				for _, item := range a.Content {
					y, err := NewDefaultType(item, compiler.NewContext("examples", item, context))
					if err != nil {
						errors = append(errors, err)
					}
		...
		// bool nullable = 56;
		v56 := compiler.MapValueForKey(m, "nullable")
		if v56 != nil {
			x.Nullable, ok = compiler.BoolForScalarNode(v56)
			if !ok {
				message := fmt.Sprintf("has unexpected value for nullable: %s", compiler.Display(v56))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// bool deprecated = 57;
		v57 := compiler.MapValueForKey(m, "deprecated")
		if v57 != nil {
			x.Deprecated, ok = compiler.BoolForScalarNode(v57)
			if !ok {
				message := fmt.Sprintf("has unexpected value for deprecated: %s", compiler.Display(v57))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// bool read_only = 58;
		v58 := compiler.MapValueForKey(m, "readOnly")
		if v58 != nil {
			x.ReadOnly, ok = compiler.BoolForScalarNode(v58)
			if !ok {
				message := fmt.Sprintf("has unexpected value for readOnly: %s", compiler.Display(v58))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// bool write_only = 59;
		v59 := compiler.MapValueForKey(m, "writeOnly")
		if v59 != nil {
			x.WriteOnly, ok = compiler.BoolForScalarNode(v59)
			if !ok {
				message := fmt.Sprintf("has unexpected value for writeOnly: %s", compiler.Display(v59))
				errors = append(errors, compiler.NewError(context, message))
			}
		}
		// repeated NamedAny specification_extension = 60;
		// MAP: Any ^x-
		x.SpecificationExtension = make([]*NamedAny, 0)
		for i := 0; i < len(m.Content); i += 2 {
			k, ok := compiler.StringForScalarNode(m.Content[i])
...
// NewUnevaluatedItemsItem creates an object of type UnevaluatedItemsItem if possible, returning an error if not.
func NewUnevaluatedItemsItem(in *yaml.Node, context *compiler.Context) (*UnevaluatedItemsItem, error) {
	errors := make([]error, 0)
	...
	// SchemaOrReference schema_or_reference = 1;
	{
		...
		if ok {
			...
			t, matchingError := NewSchemaOrReference(m, compiler.NewContext("schemaOrReference", m, context))
			if matchingError == nil {
				x.Oneof = &UnevaluatedItemsItem_SchemaOrReference{SchemaOrReference: t}
				matched = true
			} else {
				errors = append(errors, matchingError)
			}
	...
	boolValue, ok := compiler.BoolForScalarNode(in)
	if ok {
		x.Oneof = &UnevaluatedItemsItem_Boolean{Boolean: boolValue}
		matched = true
	}
	if matched {
		// since the oneof matched one of its possibilities, discard any matching errors
		...
		message := fmt.Sprintf("contains an invalid UnevaluatedItemsItem")
		err := compiler.NewError(context, message)
		errors = []error{err}
	}
	return x, compiler.NewErrorGroupOrNil(errors)
...
// NewUnevaluatedPropertiesItem creates an object of type UnevaluatedPropertiesItem if possible, returning an error if not.
func NewUnevaluatedPropertiesItem(in *yaml.Node, context *compiler.Context) (*UnevaluatedPropertiesItem, error) {
	errors := make([]error, 0)
	...
	// SchemaOrReference schema_or_reference = 1;
	{
		...
		if ok {
			...
			t, matchingError := NewSchemaOrReference(m, compiler.NewContext("schemaOrReference", m, context))
			if matchingError == nil {
				x.Oneof = &UnevaluatedPropertiesItem_SchemaOrReference{SchemaOrReference: t}
				matched = true
			} else {
				errors = append(errors, matchingError)
			}
	...
	boolValue, ok := compiler.BoolForScalarNode(in)
	if ok {
		x.Oneof = &UnevaluatedPropertiesItem_Boolean{Boolean: boolValue}
		matched = true
	}
	if matched {
		// since the oneof matched one of its possibilities, discard any matching errors
		...
		message := fmt.Sprintf("contains an invalid UnevaluatedPropertiesItem")
		err := compiler.NewError(context, message)
		errors = []error{err}
	}
	return x, compiler.NewErrorGroupOrNil(errors)
...
// NewXDefs creates an object of type XDefs if possible, returning an error if not.
func NewXDefs(in *yaml.Node, context *compiler.Context) (*XDefs, error) {
	errors := make([]error, 0)
	x := &XDefs{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
...
// NewXVocabulary creates an object of type XVocabulary if possible, returning an error if not.
func NewXVocabulary(in *yaml.Node, context *compiler.Context) (*XVocabulary, error) {
	errors := make([]error, 0)
	x := &XVocabulary{}
	m, ok := compiler.UnpackMap(in)
	if !ok {
...
func (m *AnysOrExpressions) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
...
func (m *Components) ResolveReferences(root string) (*yaml.Node, error) {
	...
	if m.PathItems != nil {
		_, err := m.PathItems.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
...
// ResolveReferences resolves references found inside DependentRequired objects.
func (m *DependentRequired) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	for _, item := range m.AdditionalProperties {
		if item != nil {
			_, err := item.ResolveReferences(root)
...
// ResolveReferences resolves references found inside DependentSchemas objects.
func (m *DependentSchemas) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	for _, item := range m.AdditionalProperties {
		if item != nil {
			_, err := item.ResolveReferences(root)
...
func (m *Document) ResolveReferences(root string) (*yaml.Node, error) {
	...
	if m.Webhooks != nil {
		_, err := m.Webhooks.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
...
func (m *NamedAnyOrExpression) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	if m.Value != nil {
		_, err := m.Value.ResolveReferences(root)
		if err != nil {
...
// ResolveReferences resolves references found inside PathItems objects.
func (m *PathItems) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	for _, item := range m.AdditionalProperties {
		if item != nil {
			_, err := item.ResolveReferences(root)
...
func (m *Schema) ResolveReferences(root string) (*yaml.Node, error) {
	...
	if m.Const != nil {
		_, err := m.Const.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.XVocabulary != nil {
		_, err := m.XVocabulary.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.XDefs != nil {
		_, err := m.XDefs.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.PropertyNames != nil {
		_, err := m.PropertyNames.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	for _, item := range m.PrefixItems {
		if item != nil {
			_, err := item.ResolveReferences(root)
			if err != nil {
				errors = append(errors, err)
	...
	if m.Contains != nil {
		_, err := m.Contains.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.If != nil {
		_, err := m.If.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.Then != nil {
		_, err := m.Then.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.Else != nil {
		_, err := m.Else.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.DependentSchemas != nil {
		_, err := m.DependentSchemas.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.DependentRequired != nil {
		_, err := m.DependentRequired.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.UnevaluatedItems != nil {
		_, err := m.UnevaluatedItems.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
	if m.UnevaluatedProperties != nil {
		_, err := m.UnevaluatedProperties.ResolveReferences(root)
		if err != nil {
			errors = append(errors, err)
		}
	}
...
// ResolveReferences resolves references found inside UnevaluatedItemsItem objects.
func (m *UnevaluatedItemsItem) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	{
		p, ok := m.Oneof.(*UnevaluatedItemsItem_SchemaOrReference)
		if ok {
			_, err := p.SchemaOrReference.ResolveReferences(root)
			if err != nil {
				return nil, err
...
// ResolveReferences resolves references found inside UnevaluatedPropertiesItem objects.
func (m *UnevaluatedPropertiesItem) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	{
		p, ok := m.Oneof.(*UnevaluatedPropertiesItem_SchemaOrReference)
		if ok {
			_, err := p.SchemaOrReference.ResolveReferences(root)
			if err != nil {
				return nil, err
...
// ResolveReferences resolves references found inside XDefs objects.
func (m *XDefs) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	for _, item := range m.AdditionalProperties {
		if item != nil {
			_, err := item.ResolveReferences(root)
...
// ResolveReferences resolves references found inside XVocabulary objects.
func (m *XVocabulary) ResolveReferences(root string) (*yaml.Node, error) {
	errors := make([]error, 0)
	return nil, compiler.NewErrorGroupOrNil(errors)
}
...
func (m *AnysOrExpressions) ToRawInfo() *yaml.Node {
...
func (m *Components) ToRawInfo() *yaml.Node {
	...
	if m.PathItems != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("pathItems"))
		info.Content = append(info.Content, m.PathItems.ToRawInfo())
	}
	if m.SpecificationExtension != nil {
		for _, item := range m.SpecificationExtension {
			info.Content = append(info.Content, compiler.NewScalarNodeForString(item.Name))
...
// ToRawInfo returns a description of DependentRequired suitable for JSON or YAML export.
func (m *DependentRequired) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
...
// ToRawInfo returns a description of DependentSchemas suitable for JSON or YAML export.
func (m *DependentSchemas) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
...
func (m *Document) ToRawInfo() *yaml.Node {
	...
	if m.JsonSchemaDialect != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("jsonSchemaDialect"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.JsonSchemaDialect))
	}
	if len(m.Servers) != 0 {
	...
	if m.Webhooks != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("webhooks"))
		info.Content = append(info.Content, m.Webhooks.ToRawInfo())
	}
	if m.Components != nil {
...
func (m *License) ToRawInfo() *yaml.Node {
	...
	info.Content = append(info.Content, compiler.NewScalarNodeForString(m.Name))
	if m.Identifier != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("identifier"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.Identifier))
	}
	if m.Url != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("url"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.Url))
...
func (m *NamedAnyOrExpression) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
...
// ToRawInfo returns a description of PathItems suitable for JSON or YAML export.
func (m *PathItems) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
...
func (m *Schema) ToRawInfo() *yaml.Node {
	...
	if m.XId != "" {
		...
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XId))
	}
	if m.XAnchor != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$anchor"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XAnchor))
	}
	if m.XComment != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$comment"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XComment))
	}
	if m.XSchema != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$schema"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XSchema))
	}
	if m.XDynamicRef != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$dynamicRef"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XDynamicRef))
	}
	if m.XDynamicAnchor != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$dynamicAnchor"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XDynamicAnchor))
	}
	if m.XRecursiveRef != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$recursiveRef"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.XRecursiveRef))
	}
	if m.XRecursiveAnchor != false {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$recursiveAnchor"))
		info.Content = append(info.Content, compiler.NewScalarNodeForBool(m.XRecursiveAnchor))
	}
	if m.XVocabulary != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$vocabulary"))
		info.Content = append(info.Content, m.XVocabulary.ToRawInfo())
	}
	if m.XDefs != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("$defs"))
		info.Content = append(info.Content, m.XDefs.ToRawInfo())
	}
	if m.PropertyNames != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("propertyNames"))
		info.Content = append(info.Content, m.PropertyNames.ToRawInfo())
	}
	if len(m.PrefixItems) != 0 {
		items := compiler.NewSequenceNode()
		for _, item := range m.PrefixItems {
			items.Content = append(items.Content, item.ToRawInfo())
		}
		info.Content = append(info.Content, compiler.NewScalarNodeForString("prefixItems"))
		info.Content = append(info.Content, items)
	}
	if m.Contains != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("contains"))
		info.Content = append(info.Content, m.Contains.ToRawInfo())
	}
	if m.If != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("if"))
		info.Content = append(info.Content, m.If.ToRawInfo())
	}
	if m.Then != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("then"))
		info.Content = append(info.Content, m.Then.ToRawInfo())
	}
	if m.Else != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("else"))
		info.Content = append(info.Content, m.Else.ToRawInfo())
	}
	if m.DependentSchemas != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("dependentSchemas"))
		info.Content = append(info.Content, m.DependentSchemas.ToRawInfo())
	}
	if m.DependentRequired != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("dependentRequired"))
		info.Content = append(info.Content, m.DependentRequired.ToRawInfo())
	}
	if m.ContentEncoding != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("contentEncoding"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.ContentEncoding))
	}
	if m.ContentMediaType != "" {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("contentMediaType"))
		info.Content = append(info.Content, compiler.NewScalarNodeForString(m.ContentMediaType))
	}
	if m.UnevaluatedItems != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("unevaluatedItems"))
		info.Content = append(info.Content, m.UnevaluatedItems.ToRawInfo())
	}
	if m.UnevaluatedProperties != nil {
		info.Content = append(info.Content, compiler.NewScalarNodeForString("unevaluatedProperties"))
		info.Content = append(info.Content, m.UnevaluatedProperties.ToRawInfo())
	}
	if len(m.Examples) != 0 {
		items := compiler.NewSequenceNode()
		for _, item := range m.Examples {
...
// ToRawInfo returns a description of UnevaluatedItemsItem suitable for JSON or YAML export.
func (m *UnevaluatedItemsItem) ToRawInfo() *yaml.Node {
	// ONE OF WRAPPER
	// UnevaluatedItemsItem
	// {Name:schemaOrReference Type:SchemaOrReference StringEnumValues:[] MapType: Repeated:false Pattern: Implicit:false Description:}
	v0 := m.GetSchemaOrReference()
	if v0 != nil {
		return v0.ToRawInfo()
	...
	// {Name:boolean Type:bool StringEnumValues:[] MapType: Repeated:false Pattern: Implicit:false Description:}
	if v1, ok := m.GetOneof().(*UnevaluatedItemsItem_Boolean); ok {
		return compiler.NewScalarNodeForBool(v1.Boolean)
	}
	return compiler.NewNullNode()
}

// ToRawInfo returns a description of UnevaluatedPropertiesItem suitable for JSON or YAML export.
func (m *UnevaluatedPropertiesItem) ToRawInfo() *yaml.Node {
	// ONE OF WRAPPER
	// UnevaluatedPropertiesItem
	// {Name:schemaOrReference Type:SchemaOrReference StringEnumValues:[] MapType: Repeated:false Pattern: Implicit:false Description:}
	v0 := m.GetSchemaOrReference()
	if v0 != nil {
		return v0.ToRawInfo()
	...
	// {Name:boolean Type:bool StringEnumValues:[] MapType: Repeated:false Pattern: Implicit:false Description:}
	if v1, ok := m.GetOneof().(*UnevaluatedPropertiesItem_Boolean); ok {
		return compiler.NewScalarNodeForBool(v1.Boolean)
	}
	return compiler.NewNullNode()
}

// ToRawInfo returns a description of XDefs suitable for JSON or YAML export.
func (m *XDefs) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
...
// ToRawInfo returns a description of XVocabulary suitable for JSON or YAML export.
func (m *XVocabulary) ToRawInfo() *yaml.Node {
	info := compiler.NewMappingNode()
	if m == nil {
		return info
	}
