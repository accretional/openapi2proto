...
import (
	...
	"github.com/accretional/openapi2proto/internal/printer"
)
...
func (domain *Domain) generateConstructorForType(code *printer.Code, typeName string, regexPatterns *patternNames) {
	...
	if typeModel.IsStringArray {
		...
		// v3 only (we only generate for v3)
		...
		if unpackAtTop {
			...
			code.Print("  errors = append(errors, compiler.NewError(context, message))")
			code.Print("} else {")
			// Suppress unused variable warning when type has no properties.
		...
		for _, propertyModel := range typeModel.Properties {
			...
			if displayName == "$ref" {
				displayName = "_ref"
			} else if displayName == "$schema" {
				displayName = "_schema"
			} else if strings.HasPrefix(displayName, "$") {
				displayName = "_" + displayName[1:]
			}
			...
			fieldName := propertyModel.FieldName()
