...
// Domain models a collection of types defined by a schema.
type Domain struct {
	TypeModels            map[string]*TypeModel
	Prefix                string
	Schema                *jsonschema.Schema
	TypeNameOverrides     map[string]string
	PropertyNameOverrides map[string]string
	ObjectTypeRequests    map[string]*TypeRequest
	MapTypeRequests       map[string]string
	Version               string
}
...
// TypeNameForStub returns a capitalized name for a generated type.
func (domain *Domain) TypeNameForStub(stub string) string {
	// Strip $ prefix and use "X" prefix for valid Go/proto identifiers.
	if strings.HasPrefix(stub, "$") {
		stub = stub[1:]
		return domain.Prefix + "X" + strings.ToUpper(stub[0:1]) + stub[1:]
	}
		...
		for _, pair := range *(schema.Properties) {
			...
			if propertySchema.Ref != nil {
				...
				if propertySchema.TypeIs("string") {
					...
					log.Printf("ignoring %+v, unsupported property type '%s'", propertyName, propertySchema.Type.Description())
				}
...
func (domain *Domain) Build() (err error) {
	...
	// iterate over anonymous object types to be instantiated
	...
	// iterate over map item types to be instantiated
